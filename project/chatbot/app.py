...
def _build_html_elements(html: str):
    """Build HtmlPreview sidebar element list if html is present."""
    if not html:
        return []
    return [
        cl.CustomElement(
            name="HtmlPreview",  # Must match public/elements/HtmlPreview.jsx
            props={
                "html": html,
                "minHeight": 200,
                "maxHeight": 1200,
...
async def _render_event(event: dict):
    ...
    # For display="side", the element name must appear in the content as a clickable link.
    if t == "message":
        content = f"{text}\n\nHtmlPreview" if html else text
